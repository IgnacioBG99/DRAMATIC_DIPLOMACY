...
// Stage 3: Artwork Selection (Full Painting Shape Respected + Subtitle Code Added)
function loadStage3() {
  const config = gameData[getTourKey()];
  ...
  if (rosterGrid && config && config.artRoster) {
    ...
    config.artRoster.forEach(art => {
      ...
      // Image box with object-fit:contain to keep exact painting shapes + file ID subtitle below
      card.innerHTML = `
