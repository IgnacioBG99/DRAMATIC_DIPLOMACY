...
function updateTourBadges() {
  badges.tour1.className = `tour-badge ${currentTour === 1 ? 'active' : currentTour > 1 ? 'completed' : 'locked'}`;
  badges.tour2.className = `tour-badge ${currentTour === 2 ? 'active' : currentTour > 2 ? 'completed' : 'locked'}`;
  badges.tour3.className = `tour-badge ${currentTour === 3 ? 'active' : currentTour > 3 ? 'completed' : 'locked'}`;
}
...
function evaluateTour() {
  ...
  if (failingPiece) {
    ...
    if (currentTour < 3) {
      ...
      resultTitle.innerText = `TOUR ${currentTour} SUCCESSFUL`;
      ...
      // FINAL VICTORY (Completed Tour 3)
      resultCard.className = "result-card success";
...
// 6. EVENT LISTENERS
document.getElementById('btn-start-game').addEventListener('click', startTour);
