...
let allSelectedArtworks = [];   // Accumulates selected artworks across Tours 1, 2, and 3
let allUnselectedArtworks = []; // Accumulates unselected artworks across Tours 1, 2, and 3
...
function startTour() {
  currentTour = 1;
  selectedArtworks = [];
  allSelectedArtworks = [];   // Reset full catalogue on fresh start
  allUnselectedArtworks = []; // Reset full unselected list on fresh start
...
// Stage 3: Selection (STRICTLY HIDDEN ARTWORK METADATA DURING SELECTION)
function loadStage3() {
  const config = gameData[getTourKey()];
...
function evaluateTour() {
  ...
  if (failingPiece) {
    ...
    // Calculate unselected artworks for this current tour
    const unselectedThisTour = config.artRoster.filter(
      art => !selectedArtworks.some(selected => selected.id === art.id)
    );

    // Save choices to global tracking arrays
    allSelectedArtworks = allSelectedArtworks.concat(selectedArtworks);
    allUnselectedArtworks = allUnselectedArtworks.concat(unselectedThisTour);
    ...
    if (currentTour < 3) {
      ...
      // REVEALS ALL SELECTED AND ALL UNSELECTED ARTWORKS ACROSS TOURS 1, 2, & 3
      // ------------------------------------------------------------------
      if (resultCard) resultCard.className = "result-card success";
      ...
      let fullRevealHTML = `
        <div style="margin-top:20px; text-align:left;">
          <h2 style="text-align:center; color:var(--cathay-teal, #008080); margin-bottom:20px; border-bottom:1px solid #333; padding-bottom:10px;">
            🏛️ Final Curatorial Audit (All Tours)
          </h2>

          <!-- SECTION 1: CHOSEN ARTWORKS -->
          <h3 style="color:#2ec4b6; margin-bottom:10px; display:flex; align-items:center; gap:8px;">
            <span>✅ Selected Master Catalogue</span> 
            <span style="font-size:0.8em; opacity:0.8;">(${allSelectedArtworks.length} Artworks Exhibited)</span>
          </h3>
          <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap:12px; max-height:300px; overflow-y:auto; padding-right:8px; margin-bottom:30px; background:rgba(0,0,0,0.3); padding:10px; border-radius:6px; border:1px solid #2ec4b633;">
      `;

      allSelectedArtworks.forEach(art => {
        fullRevealHTML += `
          <div style="background:#111; border:1px solid #2ec4b6; border-radius:6px; padding:10px; text-align:center;">
            <div style="height:90px; overflow:hidden; border-radius:4px; margin-bottom:6px; background:#000; display:flex; align-items:center; justify-content:center;">
              <img src="images/art${art.id}.jpg" alt="art${art.id}.jpg" style="max-height:100%; max-width:100%; object-fit:contain;" onerror="this.src='https://via.placeholder.com/90?text=art${art.id}.jpg';">
            </div>
            <strong style="display:block; font-size:0.8em; color:#fff; margin-bottom:2px; line-height:1.2;">"${art.title}"</strong>
            <span style="display:block; font-size:0.75em; color:#2ec4b6;"><strong>Artist:</strong> ${art.author}</span>
      ...
      fullRevealHTML += `
          ...
          <!-- SECTION 2: UNCHOSEN / LEFT IN WAREHOUSE ARTWORKS -->
          <h3 style="color:#e76f51; margin-bottom:10px; display:flex; align-items:center; gap:8px;">
            <span>📦 Warehouse Reserve</span> 
            <span style="font-size:0.8em; opacity:0.8;">(${allUnselectedArtworks.length} Artworks Not Selected)</span>
          </h3>
          <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap:12px; max-height:300px; overflow-y:auto; padding-right:8px; background:rgba(0,0,0,0.3); padding:10px; border-radius:6px; border:1px solid #e76f5133;">
      `;

      allUnselectedArtworks.forEach(art => {
        const riskBadge = art.risk 
          ? `<span style="display:inline-block; font-size:0.65em; background:#ff5555; color:#fff; padding:1px 4px; border-radius:3px; margin-top:2px;">SENSITIVE RISK</span>` 
          : `<span style="display:inline-block; font-size:0.65em; background:#333; color:#aaa; padding:1px 4px; border-radius:3px; margin-top:2px;">SAFE</span>`;

        fullRevealHTML += `
          <div style="background:#111; border:1px solid #444; border-radius:6px; padding:10px; text-align:center; opacity:0.85;">
            <div style="height:90px; overflow:hidden; border-radius:4px; margin-bottom:6px; background:#000; display:flex; align-items:center; justify-content:center;">
              <img src="images/art${art.id}.jpg" alt="art${art.id}.jpg" style="max-height:100%; max-width:100%; object-fit:contain;" onerror="this.src='https://via.placeholder.com/90?text=art${art.id}.jpg';">
            </div>
            <strong style="display:block; font-size:0.8em; color:#ddd; margin-bottom:2px; line-height:1.2;">"${art.title}"</strong>
            <span style="display:block; font-size:0.75em; color:#e76f51;"><strong>Artist:</strong> ${art.author}</span>
            <span style="display:block; font-size:0.7em; color:#888;"><strong>Year:</strong> ${art.year}</span>
            ${riskBadge}
          </div>
        `;
      });
