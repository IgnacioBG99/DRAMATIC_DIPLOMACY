...
const gameData = {
  tour1: {
    ...
    welcomeStory: "Ah, Ms./Mr. Curator! Welcome to the Urban Council. Ready to make history, or at least curate it?\n\nOur Colonial Administrator, Sir Alexander Grantham, wants to improve Hong Kong's international communication and influence. He saw how the Americans use travelling art exhibitions to win hearts and minds, and now he wants us to do something similar.\n\nSo, the mission falls on you: dig through the chaos in the City Hall Museum and Art Gallery warehouse, dust off our best pieces, and assemble an exhibition fit for the Cold War stage. Ready to get to work?",
  ...
  tour2: {
    ...
    successMessage: "Ms./Mr. Curator! Outstanding work on the first show. You made us look brilliant. Now, ready for round two?\n\nOur new Governor, Sir David C. C. Trench, has arrived. As Cold War tensions rise, he's keen to project image, influence, and Western stability. He wants another blockbuster exhibition, and he wants it yesterday.\n\nYour mission: return to the City Hall Art Gallery and Museum warehouse, select our finest international pieces, and curate an exhibition that shows the world Hong Kong is the cultural capital of Asia. Good luck!",
...
// Stage 3: Artwork Selection (STRICT VISIBILITY: ONLY IMAGE THUMBNAILS SHOWN)
function loadStage3() {
  const config = gameData[getTourKey()];
  ...
  if (rosterGrid && config && config.artRoster) {
    ...
    config.artRoster.forEach(art => {
      ...
      // STRICT RULE: Only image thumbnail rendered. No title, no author name, no details.
      card.innerHTML = `
        <div style="height:140px; width:100%; overflow:hidden; border-radius:4px; display:flex; align-items:center; justify-content:center; background:#111;">
          <img src="images/art${art.id}.jpg" alt="Artwork Thumbnail" style="width:100%; height:100%; object-fit:cover;" onerror="this.src='https://via.placeholder.com/150?text=Artwork+${art.id}';">
        </div>
      `;
...
function evaluateTour() {
  ...
  if (failingPiece) {
    // FAILURE: REVEALS ONLY TITLE AND AUTHOR OF THE TRIGGERING PIECE
    ...
    if (revealBox) {
      revealBox.innerHTML = `
        ...
        <strong>${failingPiece.name}</strong> (by ${failingPiece.author})<br>
    ...
    // SUCCESS
    if (currentTour < 3) {
      const nextTourKey = `tour${currentTour + 1}`;
      ...
      if (resultMsg) {
        resultMsg.innerText = nextConfig.successMessage;
      }
      ...
      if (actionBtn) {
        ...
        actionBtn.onclick = () => {
          ...
          activeTerritory = null;
          updateTourBadges();
          loadStage1(); // Proceeds directly to Stage 1 (Geopolitical Briefing)
        };
      }
    } else {
      ...
      // FINAL VICTORY GALLERY: UNLOCKS ALL THUMBNAILS, TITLES, AUTHORS & DETAILS
      let fullRevealHTML = `
      ...
      selectedArtworks.forEach(art => {
        fullRevealHTML += `
          <div style="background:#111; border:1px solid #333; border-radius:6px; padding:8px; text-align:center;">
            <div style="height:90px; overflow:hidden; border-radius:4px; margin-bottom:6px; background:#000;">
              <img src="images/art${art.id}.jpg" alt="${art.name}" style="width:100%; height:100%; object-fit:cover;" onerror="this.src='https://via.placeholder.com/100?text=Artwork+${art.id}';">
            </div>
            <strong style="display:block; font-size:0.85em; color:#fff; margin-bottom:2px;">${art.name}</strong>
            <span style="display:block; font-size:0.75em; color:#00a896; margin-bottom:4px; font-style:italic;">By ${art.author}</span>
